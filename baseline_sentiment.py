...
tweet_sentiment_vectors = np.vstack([np.array(list(analyzer.polarity_scores(tweet).values())) for tweet in df['Tweet']])

tweet_df = pd.concat([tweet_df, pd.DataFrame(tweet_sentiment_vectors)], axis=1)
...
pred_df.to_csv('logistic_predictions_bis.csv', index=False)
