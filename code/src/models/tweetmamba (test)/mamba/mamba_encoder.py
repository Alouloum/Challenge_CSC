...
#----------------------------------------------------------------------
# Adapted from https://github.com/alxndrTL/mamba.py/blob/main/mambapy/
#----------------------------------------------------------------------
...
class AuMBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        #Use official implementation of selective_scan
    ...
    # -------------------------- inference -------------------------- #
    #This does not work wwith a bidirectional Mamba
    """
    ...
    As we need one such cache variable per layer, we store a caches object, which is simply a list of cache object. 
    """
    ...
    def step(self, x, cache):
        ...
        #Forward SSM
        y, h = self.ssm_step(x, h,A_log=self.A_log, D = self.D, x_proj=self.x_proj, dt_proj=self.dt_proj)
        
        #Backward SSM
        if self.config.bidirectional:
            raise ValueError("Step is not available when the model is bidirectionnal.") 
        # z branch
        z = F.silu(z)
    ...
    def ssm_step(self, x, h, A_log, D, x_proj, dt_proj):
        # x : (B, ED)
        # h : (B, ED, N)
...
if __name__ == "__main__":  
    print("Test MambaEncoder")
    import os 
    device = torch.device("cpu")
    config = MambaConfig(d_model=2, n_layers=2,use_cuda=False)
    model = MambaEncoder(config)
    model.to(device)
