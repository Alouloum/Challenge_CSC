...
def generate_proba_bert(name):
    print("Generating probabilities for "+name+ " data...")
    # Load and preprocess data
    df = import_data("./challenge_data/"+name+"_tweets/")
    tqdm.pandas()
    ...
    probabilities_df.to_csv("./results/"+name+"_bert_probabilities.csv", index=False)
    print(name +" Probabilities saved")


generate_proba_bert("train")
generate_proba_bert("eval")